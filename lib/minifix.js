...
import { minifyContent } from './handler';
import { bgBlue, bgMagenta, textBlue, textMagenta, textRed } from './ANSI';
import { debounce, ensureDirectoryExistence, getElapsedTime } from './utils';
...
export const minifyFile = async (filename = '') => {
    for (const [index, value] of inputFile.entries()) {
        ...
        try {
            ...
            const minifiedCode = await minifyContent(code, fileType);
